...
// FIXME: Refactor to not use this parameter preprocessToken. It is kind of confusing and it should be possible to do in another way
  function preprocessReadToken(skipComments, preprocessToken, processMacros, onlyTransformMacroArguments) {
    ...
    if (isIdentifierStart(code, options.ecmaVersion >= 6) || (code === 92 /* '\' */ && input.charCodeAt(tokPos +1) === 117 /* 'u' */)) return preprocessReadWord(processMacros);
...
// FIXME: Find out if this is really used?
  ...
  function readToken(forceRegexp, onlyTransformMacroArguments, stealth) {
    ...
    var code = fullCharCodeAtPos(tokPos);
    // Identifier or keyword. '\uXXXX' sequences are allowed in
    // identifiers, so '\' also dispatches to that.
    if (isIdentifierStart(code, options.ecmaVersion >= 6) || code === 92 /* '\' */) return readWord(null, onlyTransformMacroArguments, forceRegexp);
  ...
  function fullCharCodeAtPos(pos) {
  ...
  function readHexNumber(finisher) {
    ...
    if (isIdentifierStart(fullCharCodeAtPos(tokPos), options.ecmaVersion >= 6)) raise(tokPos, "Identifier directly after number");
  ...
  function readNumber(startsWithDot, finisher) {
    ...
    if (isIdentifierStart(fullCharCodeAtPos(tokPos))) raise(tokPos, "Identifier directly after number");
...
// Read a string value, interpreting backslash-escapes.
  ...
  function readCodePoint() {
    let ch = input.charCodeAt(tokPos), code
    ...
    if (ch === 123) { // '{'
      ...
      ++tokPos
      if (code > 0x10FFFF) raise(codePos, "Code point out of bounds")
    } else {
      code = readHexChar(4)
    }
    return code
  ...
  function readWord1() {
    containsEsc = false
    let word = "", first = true, chunkStart = tokPos
    let astral = options.ecmaVersion >= 6
    while (tokPos < input.length) {
      let ch = fullCharCodeAtPos(tokPos)
      if (isIdentifierChar(ch, astral)) {
        tokPos += ch <= 0xffff ? 1 : 2
      } else if (ch === 92) { // "\"
        containsEsc = true
...
// Tests whether parsed token is a contextual keyword.
  ...
  function isContextual(name) {
    return tokType === _name && tokVal === name && !containsEsc;
  }
...
// Asserts that following token is given contextual keyword.
  ...
  function canInsertSemicolon() {
    return !options.strictSemicolons &&
      (tokType === _eof || tokType === _braceR || newline.test(lastEndInput.slice(lastEnd, lastEndOfFile || tokFirstStart)) ||
        (nodeMessageSendObjectExpression && nodeMessageSendObjectExpression.canInsertSemicolonBefore && options.objj) || lastEndOfFile != null);
  }
  ...
  function semicolon() {
    if (!eat(_semi) && !canInsertSemicolon()) raise((nodeMessageSendObjectExpression && options.objj && nodeMessageSendObjectExpression.start) || tokStart, "Expected a semicolon");
  }
  ...
  function isAsyncFunction() {
    ...
    skipWhiteSpace.lastIndex = tokPos;
    var skip = skipWhiteSpace.exec(input);
    var next = tokPos + skip[0].length, after;
    return !lineBreak.test(input.slice(tokPos, next)) &&
      input.slice(next, next + 8) === "function" &&
      (next + 8 === input.length ||
       !(isIdentifierChar(after = input.charCodeAt(next + 8)) || after > 0xd7ff && after < 0xdc00));
  }
  ...
  function parseMethodDeclaration(node) {
    ...
    if (eat(_parenL)) {
      ...
      if (!eat(_parenR)) {
        node.returntype = parseObjectiveJType(typeNode, true);
  ...
  function parseClassElement() {
    ...
    if (tokVal === '+' || tokVal === '-') {
      ...
      var oldInFunc = inFunction, oldLabels = labels, oldAsync = functionIsAsync;
      inFunction = true; labels = [], functionIsAsync = !!element.returntype && !!element.returntype.async;
      element.body = parseBlock(true);
      inFunction = oldInFunc; labels = oldLabels; functionIsAsync = oldAsync;
  ...
  function parseMaybeUnary() {
    if (inFunction && functionIsAsync && isContextual("await")) {
      return parseAwait();
    } else if (tokType.prefix) {
  ...
  function parseSubscripts(base, noCalls) {
    if (eat(_dot)) {
      ...
      if (options.objj) var messageSendNode = startNode(), canInsertSemi = canInsertSemicolon();
      if (eat(_bracketL)) {
  ...
  function parseExprAtom() {
    ...
    case _name:
      var id = parseIdent();
      if (options.ecmaVersion >= 8 && !containsEsc && id.name === "async" && !canInsertSemicolon() && eat(_function)) {
        return parseFunction(startNodeFrom(id), false, true);
      }
      return id;
  ...
  function parseFunction(node, isStatement, isAsync) {
    ...
    // flag (restore them to their old value afterwards).
    var oldInFunc = inFunction, oldLabels = labels, oldAsync = functionIsAsync;
    inFunction = true; labels = [], functionIsAsync = !!isAsync;
  ...
  function parseIdent(liberal) {
    var node = startNode();
    var name = tokType === _name ? tokVal : (((liberal && !options.forbidReserved) || tokType.okAsIdent) && tokType.keyword) || unexpected();
    node.name = name;
    tokRegexpAllowed = false;
    next();
    if (!liberal) {
      if (functionIsAsync && name === "await") raise(node.start, "Cannot use 'await' as identifier inside an async function");
      //if (!functionIsAsync && name === "await") raise(node.start, "Cannot use keyword 'await' outside an async function")
  ...
  function parseObjectiveJType(startFrom, canBeAsync) {
    var node = startFrom ? startNodeFrom(startFrom) : startNode(), allowProtocol = false;
    if (canBeAsync && options.ecmaVersion >= 8 && eatContextual("async")) {
      node.async = true;
    }
    if (tokType === _name) {
